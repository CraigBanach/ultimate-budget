...
export const getEntries = async (): Promise<Array<Entry>> => {
  ...
  const rows = (
    await client.sql<EntryRow>`SELECT e.id, e.description, e.amount, cr.name AS account_name, cr.id AS account_id FROM entries e INNER JOIN category_rows cr ON e.account_id = cr.id;`
  ).rows;
